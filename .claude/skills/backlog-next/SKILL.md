...
Run the status helper to get the current state:

```bash
node .codex/skills/backlog-next/scripts/next-backlog-status.mjs --json
```

This returns: `currentPriority`, `openItems`, `recentCommits`, `baselineNotes`, `defaultReasoning`.

Also read `docs/backlog.md` directly for full context on the next item's implementation notes and action plan.
...
Use the `model` parameter when spawning Agent subagents. Default to the `defaultReasoning` from the status helper, but override per slice when warranted.

### 4. Execute the slice

Spawn a subagent (Agent tool) for the slice with a clear, self-contained prompt that includes:
...
- A slice needs a product decision or user input — use AskUserQuestion
