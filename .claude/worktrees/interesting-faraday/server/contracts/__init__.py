...
class WaveguideParamsRequest(BaseModel):
    ...
    n_angular: int = 100
    n_length: int = 20
    # Legacy import field: accepted for config compatibility, always coerced to 1234.
    # OCC solve/export is always full-domain; partial-domain values are silently ignored.
    quadrants: int = 1234

    @field_validator("quadrants", mode="before")
    @classmethod
    def _coerce_quadrants_to_full_domain(cls, v: object) -> int:
        """Accept any legacy Mesh.Quadrants value but always return 1234 (full domain)."""
        return 1234
