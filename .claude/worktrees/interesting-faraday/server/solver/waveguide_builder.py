...
def _compute_phi_values(n_angular: int, quadrants: int = 1234) -> np.ndarray:
    """Compute angular sample positions for a full-circle domain.

    OCC solve/export always builds full-domain meshes (quadrants=1234).
    The ``quadrants`` argument is accepted for signature compatibility only
    and is ignored — the result is always the full-circle [0, 2π) range.
    Legacy Mesh.Quadrants import values remain tolerated in config parsing
    but do not reduce the OCC mesh domain.
    """
...
def _compute_point_grids(params: dict) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    ...
    # OCC always builds full-domain meshes; quadrants is ignored for domain reduction.
    phi_values = _compute_phi_values(n_angular)
...
def build_waveguide_mesh(
    ...
    *,
    ...
    # OCC always builds full-domain meshes; quadrants param is ignored for domain reduction.
    closed = True
