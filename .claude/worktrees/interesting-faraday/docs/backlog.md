...
Last updated: March 21, 2026 (P1 false-green readiness complete; P2 legacy mesh cleanup complete)
...
- [x] Add a host-level validation slice for Apple Silicon GPU viability: prove whether the current `bempp-cl` + OpenCL stack can run on a real GPU-backed path on Apple Silicon, or document that the maintained runtime is CPU OpenCL only / GPU unsupported. (2026-03-21: Apple Silicon has no native GPU-backed OpenCL — Metal only. `device_interface.py` returns `_APPLE_SILICON_OPENCL_UNSUPPORTED_REASON` for all OpenCL modes on arm64/aarch64. `server/README.md` §1.0 and §1.1, `docs/PROJECT_DOCUMENTATION.md` §3 document CPU-only contract. Maintained runtime is CPU OpenCL only via pocl; GPU-backed OpenCL is definitively unsupported on Apple Silicon.)
- [x] Update benchmark/preflight tooling so "ready" means a bounded solve path passes on the intended supported backend, not just dependency import + device enumeration. (2026-03-21: `bounded_solve_validation` is now a required doctor component; `summary.solveReady` gates on it; `/health.solverReady` follows `summary.solveReady`; `runtime_preflight.py` CLI description now lists bounded solve validation evidence. Done in the `feat(server): gate solve readiness on bounded validation` commit.)
- [x] Refresh docs and rerun the Tritonia repro plus `npm run test:server` and `npm test`. (2026-03-21: docs refreshed in prior commits; `npm test` 317/317 pass; `npm run test:server` 206 pass, 9 skipped, 0 fail.)
...
- [x] Define the contract explicitly: active OCC solve/export paths always build full-domain meshes (`quadrants=1234`), while legacy `Mesh.Quadrants` values remain import-compatible metadata only. (2026-03-21: `WaveguideParamsRequest._coerce_quadrants_to_full_domain` validator; `_compute_phi_values` always returns full-circle; `closed=True` always in `build_waveguide_mesh`.)
- [x] Remove or hide `quadrants` from the active solve/export UI and stop describing half/quarter BEM analysis as a supported runtime behavior. (2026-03-21: removed from `parameterInventory.js` active MESH keys; `schema.js` options reduced to `[1234]` only with updated tooltip.)
- [x] Canonicalize OCC request construction so `/api/solve` and `/api/mesh/build` no longer generate partial meshes from user-facing `quadrants` values. (2026-03-21: `waveguidePayload.js` no longer forwards `quadrants`; backend contract coerces to 1234; `simulation_runner.py` metadata uses `quadrants: 1234` constant.)
- [x] Decide legacy config handling: tolerate import of non-`1234` values, but export either `1234` or omit the field unless a dedicated compatibility mode is introduced. (2026-03-21: `mwgConfig.js` always writes `Mesh.Quadrants = 1234`; `src/config/index.js` still maps `Mesh.Quadrants` on import for compat.)
- [x] Replace tests and docs that currently assert non-full-domain OCC acceptance with full-domain-only contract coverage. (2026-03-21: `test_api_validation.py`, `waveguide-payload.test.js`, `export-module.test.js` updated; backlog updated.)
