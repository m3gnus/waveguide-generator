...
- [x] Add a host-level validation slice for Apple Silicon GPU viability: prove whether the current `bempp-cl` + OpenCL stack can run on a real GPU-backed path on Apple Silicon, or document that the maintained runtime is CPU OpenCL only / GPU unsupported. (2026-03-21: documented as unsupported — `device_interface.py` marks all Apple Silicon OpenCL modes unsupported, `test_device_interface.py` asserts this contract, and `server/README.md` / `docs/PROJECT_DOCUMENTATION.md` describe the maintained path as CPU OpenCL via pocl for bounded-repro only. No validated GPU-backed path exists.)
- [x] Update benchmark/preflight tooling so "ready" means a bounded solve path passes on the intended supported backend, not just dependency import + device enumeration. (2026-03-21: completed as part of item 4 — `runtime_preflight.py` requires `bounded_solve_validation` evidence from the persisted Tritonia bounded-solve record; `/health.solverReady` follows doctor `summary.solveReady`; test coverage in `test_runtime_preflight.py`.)
- [x] Refresh docs and rerun the Tritonia repro plus `npm run test:server` and `npm test`. (2026-03-21: `npm run test:server` → 206 tests OK (9 skipped); `npm test` → 317 tests pass (0 fail). Docs up to date.)
...
- [x] Define the contract explicitly: active OCC solve/export paths always build full-domain meshes (`quadrants=1234`), while legacy `Mesh.Quadrants` values remain import-compatible metadata only. (2026-03-21: `waveguide_builder.py` `_compute_point_grids` and `build_waveguide_mesh` now hardcode full-domain; `simulation_runner.py` emits `effectiveQuadrants: 1234` without `requestedQuadrants`.)
- [x] Remove or hide `quadrants` from the active solve/export UI and stop describing half/quarter BEM analysis as a supported runtime behavior. (2026-03-21: `quadrants` removed from `parameterInventory.js` MESH group keys; no longer exposed in the Solve & Export Mesh panel.)
- [x] Canonicalize OCC request construction so `/api/solve` and `/api/mesh/build` no longer generate partial meshes from user-facing `quadrants` values. (2026-03-21: `quadrants` removed from `waveguidePayload.js`; OCC builder ignores any `quadrants` value in params and always calls `_compute_phi_values(n_angular, 1234)`.)
- [x] Decide legacy config handling: tolerate import of non-`1234` values, but export either `1234` or omit the field unless a dedicated compatibility mode is introduced. (2026-03-21: `Mesh.Quadrants` line removed from `mwgConfig.js` export; import parsing in `src/config/index.js` and schema definition in `src/config/schema.js` kept for backward compatibility.)
- [x] Replace tests and docs that currently assert non-full-domain OCC acceptance with full-domain-only contract coverage. (2026-03-21: `test_occ_adaptive_accepts_non_full_domain_quadrants` replaced with `test_occ_adaptive_ignores_non_full_domain_quadrants_always_builds_full_domain` in `test_api_validation.py`; new test asserts `effectiveQuadrants=1234` and absence of `requestedQuadrants` in metadata.)
