...
| Q005 | All 105 named parameter keys in the centralized inventory are represented above; closely related guiding-curve keys are grouped into four rows. Was 110 in v1: FREEFORM dropped `throatTangentScaleH/V`, `mouthTangentScaleH/V`, and `overshootPolicy` when the mesher began solving the tangent speed | `src/ui/parameterInventory.js:17-243` | design-ui | P1 | parse inventory and compare C rows | Inventoried | None |
