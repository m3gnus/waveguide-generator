...
- JS geometry tessellation is **live viewport/local-export only**; actual simulation meshes are generated by HornLab mesher in the backend
- `/api/mesh/viewport` can build backend/Gmsh display triangles for parity checks, but `src/app/scene.js` currently renders the JS viewport mesh
