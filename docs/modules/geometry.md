...
- **Mesh topology**: Generate live viewport/local-export meshes with deterministic structure
- **Payload assembly**: Build canonical mesh payloads for regression checks and compatibility surfaces; runtime solve submission uses the HornLab mesher placeholder contract instead
...
**Mesh topology**:
- Frontend JS topology is **always full-domain** (not trimmed by `quadrants`); HornLab solve/export meshing happens server-side and may use selected symmetry-reduced quadrants
- HornLab mesher owns viewport geometry: `src/app/scene.js` fetches point grids + enclosure rings from `/api/mesh/viewport` and tessellates them with `src/geometry/viewportTessellator.js`; the JS engine (`src/geometry/engine/`) is the backend-down fallback and the local-export path
- Backend↔engine geometry parity is enforced by `tests/viewport-backend-parity.test.js` (the deletion gate for JS engine viewport code)
- Viewport rendering may internally duplicate `throat_disc` vertices for normal generation (crisp shading) without affecting backend solve/export meshes
