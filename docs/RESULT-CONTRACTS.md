...
| Directivity index | One dB curve: `10 log10(reference-axis mean-square pressure / full-sphere mean-square pressure)`. A spherical grid is integrated directly when present; otherwise H and V polar orbits are combined as a documented approximate full-sphere result. All averages use linear power and solid-angle cell weights. | Full sphere. Infinite-baffle hemisphere grids treat the unsampled rear hemisphere as zero radiation. A diagonal display cut is not assigned another equal share of the sphere. When the configured cuts cover only one side of each orbit, the opposite side is mirrored and this assumption is recorded in metadata. | One DI value per frequency | Missing/incomplete sphere data falls back to complete H/V cuts; fewer than both complete cuts yields `null`. DI is not clamped, so a non-principal reference axis may legitimately produce a negative value. | `server/solver/directivity_index.py`; `server/solver/result_mapping.py` |
...
10. RESOLVED 2026-08-12: public DI is one full-sphere, solid-angle-weighted linear-power ratio. The spherical grid is authoritative when available; otherwise the H/V orbit pair supplies the recognized approximate full-sphere calculation. Plane indices are not exposed as ordinary DI. Beam-shape DI uses the same integrator (`server/solver/directivity_index.py`; `server/solver/beam_shape.py`).
