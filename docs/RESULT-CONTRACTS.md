...
| Observation defaults | Polar config defaults to 2 m, mouth origin, H/V/D display planes, diagonal inclination 45°, 37 spherical theta samples, and 72 spherical phi samples. A complete spherical field is requested independently for DI; the balloon flag controls whether that field is retained in the public result. | `server/jobs/models.py`; `server/solver/result_mapping.py` |
| Mapper fallback | If observation config is absent, the mapper falls back to mouth origin, 2 m, H/V arcs, and 37 angles from 0° through 180°. A backend without spherical-field support may still return those display cuts, but DI remains unavailable rather than being inferred from them. | `server/solver/result_mapping.py` |
...
| Directivity index | One dB curve: `10 log10(reference-axis mean-square pressure / full-sphere mean-square pressure)`. Only a complete spherical pressure grid is accepted. All averages use linear power and solid-angle cell weights. | Full sphere, independent of which H/V/D display cuts were requested. Infinite-baffle hemisphere grids treat the physically absent rear hemisphere as zero radiation. | One DI value per frequency | Missing, incomplete, or backend-unsupported spherical data yields `null`; polar display cuts are never substituted. DI is not clamped, so a non-principal reference axis may legitimately produce a negative value. | `server/solver/directivity_index.py`; `server/solver/result_mapping.py` |
| Spherical DI in beam shape | The same full-sphere DI definition and integrator as the primary DI curve; retained in the beam-shape object for its frequency readout | Uses the full theta/phi grid, or a front hemisphere plus the infinite-baffle zero-radiation condition | One optional value per balloon frequency | Any non-finite grid or incomplete required angular domain yields `null` for that frequency | `server/solver/directivity_index.py`; `server/solver/beam_shape.py` |
...
10. RESOLVED 2026-08-13: public DI is one full-sphere, solid-angle-weighted linear-power ratio. WG requests a spherical field independently of the selected display planes, and no H/V or per-plane approximation is exposed as DI. Beam-shape DI uses the same integrator (`server/solver/directivity_index.py`; `server/solver/beam_shape.py`).
