...
**Verdict: FAIL** *(as audited 2026-08-03; remediated 2026-08-04 — see the remediation status note below).* V2 registers all 110 claimed legacy parameter names, but it does not provide complete editable parity. The largest gaps are solve/polar submission, Freeform editing, expression entry, export management, result controls, and viewer preferences.

Audit was read-only; nothing was modified.

> **Remediation status — 2026-08-04.** The FAIL verdict below describes the state *before* the four-batch remediation wave (`e981acd` G4 viewer prefs · `f8e3757` G3 results/exports/jobs prefs · `91765df` G2 expressions/FREEFORM/solve UI · `dffdd09` G1 server contracts). The gap table is preserved unedited as the historical record; this note records what a key-by-key re-check on 2026-08-04 found.
>
> **Closed.** Of the 61 `MISSING` rows, all but the exceptions below are now present and editable in v2, under v2's own naming. The re-check followed the capability rather than the v1 spelling, since most renames are deliberate: viewer controls (`rotateSpeed`, `zoomSpeed`, `panSpeed`, `dampingEnabled`, `dampingFactor`, `invertWheelZoom`, `keyboardPanEnabled`, `startupCameraMode`, and `live-update` → `liveUpdate`) live in `frontend/src/viewerprefs/viewerPreferences.ts`; solve controls (`solverBackend` → `engine`, `meshValidationMode` → `mesh_validation_mode`, `verbose`) and the entire polar surface (`polarAngleStart/End/Step`, `polarDistance`, `polarNormAngle`, `polarDiagonalAngle` → `inclination`, `polarObservationOrigin`, `polarEnabledAxes.*`) live in `frontend/src/stores/solveOptions.ts` as `PolarUiState` → `polar_config`; `chartTheme`, `defaultSort` → `jobSort`, `minRatingFilter` → `minRating`, and `downloadSimMesh` → `autoDownloadMesh` live in `frontend/src/prefs/preferences.ts`; `SolveRequest.label` is submitted from `frontend/src/jobs/actions.ts:77` and validated at `server/jobs/models.py:144`.
>
> **The 43 `†` expression rows are closed.** Expression entry is implemented generically: `NumberField` accepts a non-numeric ATH expression when `allowExpression` is set, `ParamPanel` sets it per field via `fieldAcceptsExpression`, and the raw spelling round-trips losslessly through the `_expressions` sidecar, which is stripped from the wire payload (`frontend/src/stores/design.ts`; tests `NumberField.test.tsx:89` and `design.serialize.test.ts:53`).
>
> **Superseded rather than closed — four rows.** `resultsLayout`, `panelMode`, and `panelArrangement` have no v2 equivalent by design: Dockview replaces v1's fixed layout modes with draggable, resizable, persisted panels. The FREEFORM *profile editor* rows are likewise partial — v2 provides data parity through editable point and station tables plus paste import (`frontend/src/design/FreeformEditors.tsx`), but not v1's graphical editor affordances: H/V-plane visibility toggles and the cross-section inset depth scrubber are absent. Those are view-only controls with no effect on the design document, so parity of *inputs* holds; parity of *interaction* does not. Confirm or close that deliberately.

Sources: [v1 inventory](<https://github.com/m3gnus/waveguide-generator/blob/v1/src/ui/parameterInventory.js:1>), [v1 schema](<https://github.com/m3gnus/waveguide-generator/blob/v1/src/config/schema.js:1>), [v1 parameter UI](<https://github.com/m3gnus/waveguide-generator/blob/v1/src/ui/paramPanel.js:501>), [v1 settings](<https://github.com/m3gnus/waveguide-generator/blob/v1/src/ui/settings/modal.js:550>), [v1 polar UI](<https://github.com/m3gnus/waveguide-generator/blob/v1/src/ui/simulation/polarSettings.js:1>), [v2 registry](frontend/src/design/parameterRegistry.ts#L73), [v2 panel](frontend/src/design/ParamPanel.tsx#L50), [v2 solve submission](frontend/src/jobs/actions.ts#L54), [v2 request schema](server/jobs/models.py#L18).
