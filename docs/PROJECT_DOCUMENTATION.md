...
2. For `OSSE` / `R-OSSE`, export prefers `POST /api/mesh/build` (OCC path).
3. If OCC endpoint returns `503`, export falls back to JS `.geo` + `POST /api/mesh/generate-msh`.
4. Bundle is assembled in-browser with `.msh` plus generated ABEC text files and `bem_mesh.geo`.
...
Frontend call path:
- `src/app/exports.js` -> `buildExportMeshWithGmsh(...)`
