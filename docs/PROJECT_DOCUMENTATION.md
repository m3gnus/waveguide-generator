...
- `.geo` generated by `src/export/gmshGeoBuilder.js`
...
The project underwent a significant architectural cleanup to streamline mesh generation and remove legacy code paths.

### 12.1 Unified Mesh Generation
- Removed legacy backend meshing logic (`gmsh_geo_mesher.py`).
- Removed legacy frontend `.geo` builders (`gmshGeoBuilder.js`).
- **Single Source of Truth**: All simulation meshes now come from the Python OCC builder.
...
- Removed obsolete endpoints (`/api/mesh/generate-msh`) and models.
