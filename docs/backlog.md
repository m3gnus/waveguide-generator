...
- [ ] Tighten frontend module boundaries and expand the boundary-test lane so `src/modules/*` remains an app-facing API layer instead of a second UI/runtime layer.
  ...
  Progress: March 12, 2026. The first boundary-hardening slice moved `src/modules/export/useCases.js` off `GlobalState`, UI file/toast helpers, and the dev-only backend diagnostic hook. The module now consumes explicit state snapshots plus `writeFile`/error callbacks from the app or UI edge, and `tests/architecture-boundaries.test.js` now locks that seam while temporarily grandfathering the still-pending simulation/UI adapters called out below. Follow-up simulation slices split pure domain helpers into `src/modules/simulation/domain.js`, then isolated the approved `GlobalState` bridge in `src/modules/simulation/state.js` and the job-shape helpers in `src/modules/simulation/jobs.js`. Subsequent UI-boundary slices moved folder-workspace refresh, feedback, file-ops, and finally settings/viewer/param-panel helpers off the old `src/modules/ui/useCases.js` pass-through surface and onto the existing `UiModule` app coordinator, after which `src/modules/ui/useCases.js` was removed. A final cleanup slice removed the stale unused `GlobalState` import from `src/modules/design/useCases.js`. The next slice moved the geometry viewport bridge onto an explicit app-state snapshot in `App`/`scene.js`, so `src/modules/geometry/useCases.js` no longer imports ambient state and the boundary test no longer needs to grandfather that edge. The latest slice finished the corresponding `UiModule` edge cleanup by moving file-ops and feedback delegation onto an explicit `src/app/uiAdapters.js` app-edge adapter, so `src/modules/ui/index.js` no longer imports `src/ui/fileOps.js` or `src/ui/feedback.js`. The remaining debt in this priority band is now `src/modules/simulation/workspaceTasks.js`, which still owns explicit folder-workspace/task-index adapters that the boundary test names directly.
  Implementation plan:
