...
- [ ] Split `src/modules/simulation/useCases.js` into smaller frontend services with single responsibilities.
  ...
  Progress: March 12, 2026. The first split slice moved folder-workspace index/manifest sync plus task-bundle file writes into `src/modules/simulation/workspaceTasks.js`, leaving `useCases.js` focused on state/domain helpers and job-shape utilities. Remaining work is the pure simulation-domain extraction so canonical mesh prep, OCC request prep, and config validation can stand on their own surface without the state facade and job builders in the same file.
  Implementation plan:
