...
- [ ] Add an "Import Mesh" button in the Project section of the actions panel (`index.html`), next to the existing "Load Config" button. Wire a hidden file input accepting `.msh` files, similar to the existing config upload pattern in `src/app/events.js`.
- [ ] Add an "imported mesh mode" flag to `GlobalState` (`src/state.js`). When active, the viewport renders the imported mesh instead of the parametric model. The parametric controls remain visible but are visually dimmed or annotated to indicate they are not driving the display.
- [ ] Feed imported vertices/indices into the existing `applyMeshToScene()` in `src/app/scene.js`. The Three.js rendering path already handles arbitrary `BufferGeometry` — no changes needed to the renderer itself.
