...
- [x] Add a bounded Tritonia-M benchmark/repro path (1-frequency and reduced sweep) that reports mesh-prep success, selected runtime/device, solver stage timings, and supported vs unsupported precision modes on the active host. (2026-03-20: `server/scripts/benchmark_solver.py --preset tritonia` now builds the Tritonia OCC mesh, runs a bounded solve matrix (`single`,`double` by default), and emits explicit per-precision support status plus runtime/timing metadata in human-readable and `--json` forms.)
