...
- [ ] Tighten frontend module boundaries and expand the boundary-test lane so `src/modules/*` remains an app-facing API layer instead of a second UI/runtime layer.
  ...
  Progress: March 12, 2026. The first boundary-hardening slice moved `src/modules/export/useCases.js` off `GlobalState`, UI file/toast helpers, and the dev-only backend diagnostic hook. The module now consumes explicit state snapshots plus `writeFile`/error callbacks from the app or UI edge, and `tests/architecture-boundaries.test.js` now locks that seam while temporarily grandfathering the still-pending simulation/UI adapters called out below. Follow-up simulation slices split pure domain helpers into `src/modules/simulation/domain.js`, then isolated the approved `GlobalState` bridge in `src/modules/simulation/state.js` and the job-shape helpers in `src/modules/simulation/jobs.js`. A later UI-boundary slice removed the `src/modules/ui/useCases.js -> src/ui/workspace/folderWorkspace.js` pass-through and now routes folder-workspace change refreshes through the existing app event bus plus the simulation-panel coordinator. The next follow-up removed the last `window` / `document` defaults from `src/modules/ui/useCases.js`, a further app-edge slice moved update/config-import feedback off `src/modules/ui/useCases.js` onto the existing `UiModule` app coordinator, and the latest slice moved the remaining config-import/folder-picker file-ops wrappers onto that same coordinator. `src/modules/ui/useCases.js` is now limited to viewer/settings/param-panel style adapters instead of file/toast/workspace helper pass-through.
  Implementation plan:
