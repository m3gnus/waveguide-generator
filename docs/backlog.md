...
- [ ] Tighten frontend module boundaries and expand the boundary-test lane so `src/modules/*` remains an app-facing API layer instead of a second UI/runtime layer.
  ...
  Research findings: `tests/architecture-boundaries.test.js` currently blocks `src/app` and `src/ui` from importing core internals, but it does not stop `src/modules/*` from importing UI/browser concerns. `src/modules/export/useCases.js` previously mixed export orchestration with `GlobalState`, `saveFile()`, `showError()`, and a `window.testBackendConnection` diagnostic surface; `src/modules/simulation/useCases.js` has now shed workspace persistence and pure domain helpers but still mixes the remaining `GlobalState` facade with job-shape helpers; `src/modules/ui/useCases.js` is mostly a pass-through wrapper over `src/ui/*`.
...
- [ ] Split `src/modules/simulation/useCases.js` into smaller frontend services with single responsibilities.
  ...
  Progress: March 12, 2026. The first split slice moved folder-workspace index/manifest sync plus task-bundle file writes into `src/modules/simulation/workspaceTasks.js`, leaving `useCases.js` focused on state/domain helpers and job-shape utilities. A second slice then extracted canonical mesh prep, OCC request prep, solver-client creation, mesh diagnostics, and config validation into `src/modules/simulation/domain.js`, and rewired the app/UI callers to use that pure state-input surface directly. Remaining work is to split the surviving state facade and job-shape helpers so `useCases.js` no longer carries two responsibilities under one file.
  Implementation plan:
