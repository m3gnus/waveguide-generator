...
function ResultsPreferencesContent() {
  ...
  return <section className="preferences-section">
    ...
    <div className="preferences-grid">
      ...
      <label className="ui-field" title="Which contour the directivity map emphasises, and how far down its colour scale runs. A level in dB — unrelated to the normalization angle, which is where 0 dB sits.">Map contour<select aria-label="Map contour" value={preferences.mapReference} onChange={(event) => preferencesStore.update({ mapReference: Number(event.target.value) as MapReference })}>{MAP_REFERENCES.map((value) => <option key={value} value={value}>{value} dB</option>)}</select></label>
