...
describe('imported solve submission wire', () => {
  ...
  // formulation is not a choice for it: whatever the parametric solver path
  // is, an imported solve is full 3-D.
  it("carries the user's engine choice and always asks for full 3-D", () => {
    useCadReturnStore.setState({
      selectedBundle: bundle,
      ingestRecord: record,
    ...
    expect(options.engine).toBe('beat-cpu');
    expect(options.solver_mode).toBe('full_3d');
    expect(options.symmetry).toBe('auto');
  });
