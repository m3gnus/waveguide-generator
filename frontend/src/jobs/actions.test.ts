...
describe('solve submission', () => {
  ...
  it('never lets AUTO silently select Axisym when no full-3D engine exists', () => {
    ...
    expect(resolveEngine('auto', axisymOnly, 'circsym')).toBe('axisym');
  ...
  it('submits the imported geometry union without a parametric design sibling', async () => {
    ...
    // cad-solve:<id> keys are the backend's, for Fusion's commands; the browser
    // sends none.
