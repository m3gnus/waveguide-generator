...
function BalloonScene({ result, frequencyIndex, resetNonce }: { result: ResultPayload; frequencyIndex: number; resetNonce: number }) {
  ...
  const geometry = useMemo(
    () => buildBalloonGeometry(balloon.theta_deg, balloon.phi_deg, balloon.spl_norm_db[frequencyIndex] ?? []),
    [balloon, frequencyIndex],
  );
  ...
  return <>
    ...
    <hemisphereLight args={['#d9efff', '#171324', 1.8]}/>
    <directionalLight position={[3, 4, 5]} intensity={2.7}/>
    <directionalLight position={[-3, -1, -2]} intensity={.9} color="#5f74ff"/>
    <mesh geometry={geometry}>
      <meshStandardMaterial vertexColors side={DoubleSide} roughness={.32} metalness={.08}/>
    </mesh>
    <mesh geometry={geometry} scale={1.003}>
      <meshBasicMaterial color="#ffffff" wireframe transparent opacity={.105} depthWrite={false}/>
    </mesh>
