...
describe('SettingsDialog', () => {
  ...
  it('points at the update dialog for the channel it used to hold', async () => {
    // The Stable/Beta buttons moved next to the verdict they decide. The
    // section stays: a setting that simply vanishes reads as one that was
    // removed, and this is where people learned to look for it.
    vi.stubGlobal('fetch', vi.fn(async () => { throw new Error('offline'); }));
    act(() => host.querySelector<HTMLButtonElement>('#open-settings')!.click());
    await act(async () => { await Promise.resolve(); await Promise.resolve(); });

    const section = host.querySelector<HTMLElement>('#settings-updates')!;
    expect(section.textContent).toContain('update dialog');
    expect(section.textContent).toContain('click the version');
    expect([...section.querySelectorAll('button')].map((button) => button.textContent))
      .not.toContain('Beta');
  });
