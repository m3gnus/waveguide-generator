...
export function StatusBar() {
  const { engines, error: capabilityError } = useCapabilities();
  const preview = useSyncExternalStore(previewSocket.subscribe, previewSocket.getSnapshot, previewSocket.getSnapshot);
  const design = useDesignStore((state) => state.design);
  const engine = engines.find((item) => item.available) ?? engines[0];
  const engineLabel = engine ? `${engine.name.toUpperCase()} · ${engine.available ? engine.version ?? 'READY' : 'OFFLINE'}` : capabilityError ? 'ENGINE OFFLINE' : 'ENGINE…';
