...
/**
 * Whether an engine declares imported CAD geometry among what it solves.
 *
 * A payload from a server older than `geometry_sources` is read as it behaved
 * then: Metal alone solved imported geometry.
 */
export function declaresImportedGeometry(engine: EngineCapability): boolean {
  return engine.geometry_sources?.includes('imported') ?? engine.name.toLowerCase() === 'metal';
}

/** The engine an imported CAD solve would run on, or why none can. */
export interface ImportedEngineChoice {
  engine: EngineCapability | null;
  reason: string | null;
}

function engineLabel(engine: EngineCapability): string {
  return engine.label || engine.name;
}

/**
 * Which engine the user's choice lands on for an imported CAD solve.
 *
 * Imported geometry takes the same engine choice as a parametric design. The
 * server makes the decision (`resolve_imported_submission`); this reads the
 * same declarations so the Solve control and the CAD rail can say where the
 * solve will run before it is pressed. AUTO walks the server's full-3D order
 * and takes the first available engine that declares imported geometry, as a
 * parametric AUTO walks it. An explicit choice is taken as it is: an engine
 * that cannot solve imported geometry is the answer, with its reason, never a
 * silent swap. The server may still refuse a particular return (a y-only half
 * on BEAT, for one), and names the reason when it does.
 */
export function importedSolveEngine(
  selected: string,
  engines: readonly EngineCapability[],
  selection?: Readonly<EngineSelection>,
): ImportedEngineChoice {
  const requested = selected.trim().toLowerCase();
  const declaring = engines.filter(declaresImportedGeometry);
  const offer = declaring.length
    ? declaring.map(engineLabel).join(', ')
    : 'none on this machine';
  if (requested === 'axisym' || requested === 'circsym') {
    return {
      engine: null,
      reason: 'Imported CAD geometry solves in full 3-D only. Choose AUTO or a full-3D engine.',
    };
  }
  if (requested === '' || requested === 'auto' || requested === LEGACY_BEAT_ENGINE) {
    const advertised = selection?.full3dOrder?.map((name) => name.toLowerCase()) ?? [];
    const order = advertised.length ? advertised : engines.map((item) => item.name.toLowerCase());
    const candidates = order
      .filter((name) => requested !== LEGACY_BEAT_ENGINE || isBeatBackendEngine(name))
      .flatMap((name) => declaring.filter((item) => item.name.toLowerCase() === name));
    const engine = candidates.find((item) => item.available) ?? null;
    if (engine) return { engine, reason: null };
    const unavailable = candidates.map((item) => `${engineLabel(item)}: ${item.reason ?? 'unavailable'}`);
    return {
      engine: null,
      reason: unavailable.length
        ? `No engine that solves imported CAD geometry is available here. ${unavailable.join('; ')}.`
        : 'No engine here solves imported CAD geometry.',
    };
  }
  const engine = engines.find((item) => item.name.toLowerCase() === requested);
  if (!engine) return { engine: null, reason: `${requested} is not an engine this server offers.` };
  if (!declaresImportedGeometry(engine)) {
    return {
      engine: null,
      reason: `${engineLabel(engine)} does not solve imported CAD geometry. Engines that do: ${offer}.`,
    };
  }
  if (!engine.available) {
    return {
      engine: null,
      reason: `${engineLabel(engine)} is unavailable${engine.reason ? `: ${engine.reason}` : ''}.`,
    };
  }
