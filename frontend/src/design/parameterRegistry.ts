...
export const PARAMETER_REGISTRY: ParameterDefinition[] = [
  ...
  number('osse.a0', 'a0', 'a0', profile, 'Throat coverage angle', { symbol: 'a0', families: ['OSSE'], unit: '°', min: -90, max: 90, description: 'Wall angle where the OS-SE curve leaves the throat, which sets how fast it starts opening. Ignored when Throat profile is Circular arc. Accepts an expression in p, such as "15 + 2*sin(p)".' }),
  ...
  number('common.throat_ext_angle', 'throatExtAngle', 'throat_ext_angle', throatExtension, 'Throat extension angle', { families: osseFamilies, unit: '°', min: -90, max: 90, description: 'Taper of the conical throat extension. The driver end works out as r0 minus the extension length times tan of this angle, so positive angles narrow it toward the driver and negative ones widen it; 0 makes a straight tube. The main horn throat is unchanged.' }),
  number('common.throat_ext_length', 'throatExtLength', 'throat_ext_length', throatExtension, 'Throat extension length', { families: osseFamilies, unit: 'mm', min: 0, max: 1_000, description: 'Length of the conical adapter added behind the throat, between the driver and the horn. 0 leaves it out. It is always added on top of Horn length.' }),
