...
export const PARAMETER_REGISTRY: ParameterDefinition[] = [
  ...
  number('morph.corner_radius', 'morphCorner', 'morph.corner_radius', morphTarget, 'Corner radius', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], unit: 'mm', min: 0, max: 100, step: 1, description: 'Rounds the corners of a Rectangle target, clamped to the smaller half-dimension of that target. It has no effect on a Circle or Superellipse target. On R-OSSE, OSSE and ICW, 0 gives a sharp rectangle. FREEFORM rejects a morph whose intermediate outlines turn non-convex, which a small corner or a target far from the drawn mouth can cause; the error names a minimum corner radius only when some radius would pass. For a rectangular FREEFORM mouth, make the mouth cross-section station a Rounded rectangle instead.' }),
