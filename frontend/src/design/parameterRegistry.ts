...
export const PARAMETER_REGISTRY: ParameterDefinition[] = [
  ...
  select('morph.target_shape', 'morphTarget', 'morph.target_shape', morphTarget, 'Morph target', [{ value: 0, label: 'None' }, { value: 1, label: 'Rectangle' }, { value: 2, label: 'Circle' }, { value: 3, label: 'Superellipse' }], { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], description: 'Outline the mouth is blended toward as it approaches the baffle. None keeps the outline the profile formula produces. A Circle takes half the larger target extent as its radius, so on an anisotropic mouth it expands the smaller axis and cannot preserve both mouth endpoints. Only Superellipse is shape-only; exponent 2 gives an ellipse.' }),
  number('morph.target_width', 'morphWidth', 'morph.target_width', morphTarget, 'Target width', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], unit: 'mm', min: 0, max: 2_000, description: 'Full target width. For Rectangle and Circle, 0 derives it from the mouth by rounding the half-width up to a whole millimetre before doubling. For Superellipse, 0 uses the exact full mouth width. A Circle uses the larger of width and height as its diameter.' }),
  number('morph.target_height', 'morphHeight', 'morph.target_height', morphTarget, 'Target height', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], unit: 'mm', min: 0, max: 2_000, description: 'Full target height. For Rectangle and Circle, 0 derives it from the mouth by rounding the half-height up to a whole millimetre before doubling. For Superellipse, 0 uses the exact full mouth height. A Circle uses the larger of width and height as its diameter.' }),
  number('morph.target_exponent', 'morphExponent', 'morph.target_exponent', morphTarget, 'Target exponent', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], min: 2, max: 16, visibleWhen: (design) => design.morph.target_shape === 3, description: 'Exponent of the Superellipse target. 2, the default, gives an ellipse; higher values square off the outline toward a rounded rectangle. An expression in the azimuthal angle p is evaluated around the mouth.' }),
  number('morph.corner_radius', 'morphCorner', 'morph.corner_radius', morphTarget, 'Corner radius', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], unit: 'mm', min: 0, max: 100, step: 1, description: 'Rounds the corners of a Rectangle target, clamped to the smaller half-dimension of that target. It has no effect on a Circle or Superellipse target. A genuinely sharp rectangle cannot be reached by morph because the intermediate outlines become non-convex and are rejected; on a measured 320 × 220 mm mouth, 10 mm or less was rejected and 15 mm or more accepted regardless of where morphing began. The mesher error reports the minimum feasible radius for the actual geometry. Use a cross-section station for a genuinely sharp rectangle.' }),
  number('morph.rate', 'morphRate', 'morph.rate', morphTarget, 'Morph rate', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], min: 0, max: 100, description: 'Exponent on the blend once morphing starts. 1 is a straight ramp; higher values hold the formula shape longer and swing to the target near the mouth; 0 jumps straight to the target.' }),
  number('morph.fixed_part', 'morphFixed', 'morph.fixed_part', morphTarget, 'Fixed part', { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], min: 0, max: 1, step: .01, description: 'Fraction of the profile from the throat left untouched before blending starts; 0 blends from the throat. 1 disables morphing altogether, as does any value at or above Truncation limit on R-OSSE. Throat extensions and slots are never morphed.' }),
  select('morph.allow_shrinkage', 'morphAllowShrinkage', 'morph.allow_shrinkage', morphTarget, 'Allow shrinkage', yesNo, { families: ['R-OSSE', 'OSSE', 'ICW', 'FREEFORM'], description: 'Whether the target may be smaller than the mouth the formula produced. No raises the target width and height to at least the mouth bounds, so a smaller target is enlarged rather than honoured.' }),
  ...
  number('mesh.vertical_offset', 'verticalOffset', 'mesh.vertical_offset', solveExportMesh, 'Export vertical offset', { unit: 'mm', min: -10_000, max: 10_000, description: 'Shifts the exported design vertically, for placing a horn off-centre on its baffle. It applies only to the solid STEP and Send to CAD exports. The viewport, the solver mesh, every acoustic result and the recentred exports (STL, surface STEP, profile CSV) are all built in the recentred frame, so simulation results never depend on it.' }),
  ...
  { id: 'passthrough.abec', legacyKey: 'ABEC', section: 'Output & Passthrough', label: 'Other ABEC passthrough blocks', kind: 'indicator', description: 'Whether the imported file carried ABEC project blocks other than the polar blocks managed by Directivity Map. These remaining blocks are not edited here and are written back unchanged.' },
...
export const EXPRESSION_PARAMETER_IDS = new Set([
  ...
  'morph.target_width', 'morph.target_height', 'morph.target_exponent', 'morph.corner_radius', 'morph.rate', 'morph.fixed_part',
