...
function createDependencyItem(component) {
  ...
  const name = component.version ? `${component.name} (${component.version})` : component.name;
...
function renderDependencyStatus(container, health) {
  ...
  container.classList.toggle('has-warnings', getRuntimeDoctorIssues(health).length > 0);
}
...
export function getFeatureBlockedReason(health, feature) {
  const featureAliases = {
    'hornlab-mesher-mesh': ['meshBuild'],
    'hornlab-mesh': ['meshBuild'],
    'occ-mesh': ['meshBuild'],
