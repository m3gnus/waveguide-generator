...
test('DesignModule backend mesh normalization outputs centralize simulation/export request prep', () => {
  ...
  assert.equal(backendMeshSimulation.maxTriangles, 24000);
  assert.equal(backendMeshSimulation.allowLargeMesh, true);
  ...
  assert.equal(JSON.stringify(directBackendMeshSimulation), JSON.stringify(backendMeshSimulation));
...
test('DesignModule normalizes explicit quadrants with ATH leading-integer fallback', () => {
  ...
  assert.equal(
    prepareBackendMeshSimulationParams({ type: 'OSSE', quadrants: '1234x' }).quadrants,
    1234
  );
