...
test("formula allowlist limits per-row formula controls to audited fields", () => {
  ...
  assert.equal(
    getControlInputMode(PARAM_SCHEMA.MESH.angularSegments),
    "number",
  );
  assert.equal(
    getControlInputMode(PARAM_SCHEMA.ENCLOSURE.encFrontResolution),
    "text",
  );
...
test("ParamPanel renders row-level formula buttons and removes the section-header affordance", () => {
  ...
  try {
    ...
    const headerButtons = collectNodes(
      coreHeader,
    ...
    const rButtons = collectNodes(
      paramContainer,
      (node) =>
        node.tagName === "BUTTON" && node.attributes["data-param-key"] === "R",
    );
    ...
    const encResolutionButtons = collectNodes(
      ...
      (node) =>
        node.tagName === "BUTTON" &&
        node.attributes["data-param-key"] === "encFrontResolution",
    );
    ...
    const sourceContourButtons = collectNodes(
      ...
      (node) =>
        node.tagName === "BUTTON" &&
        node.attributes["data-param-key"] === "sourceContours",
    );
    ...
    assert.equal(
      String(freqStartInput?.value),
      String(getDefaults("R-OSSE").freqStart),
    );
    assert.equal(
      String(freqEndInput?.value),
      String(getDefaults("R-OSSE").freqEnd),
    );
    assert.equal(
      String(freqStepsInput?.value),
      String(getDefaults("R-OSSE").numFreqs),
    );

    const simulationLabel = collectNodes(
      simulationSettingsContainer,
      (node) =>
        node.tagName === "LABEL" && node.textContent === "Sweep Start (Hz)",
    );
    ...
    const throatSliceDensityRows = collectNodes(
      simulationContainer,
      (node) =>
        node.attributes["data-param-key"] === "throatSliceDensity" &&
    ...
    assert.deepEqual(simulationSectionTitles, [
      "Source Definition",
      "Preview Mesh",
      "Solve & Export Mesh",
    ]);
