...
test('buildExportMeshWithGmsh requests backend gmsh meshing endpoint', async () => {
  ...
  try {
    ...
    assert.equal(payload.geoText.includes('.F = "'), true);
...
test('buildExportMeshWithGmsh enforces freestanding wall thickness when enclosure is disabled', async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    ...
    const result = await buildExportMeshWithGmsh(app, prepared);
    assert.ok(result.artifacts.mesh.groups?.freestandingWall);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
