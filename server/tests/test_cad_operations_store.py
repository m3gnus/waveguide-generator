...
def test_a_new_solve_supersedes_only_same_snapshot_solves_waiting_for_user(
    store: CadLinkStore,
) -> None:
    target, inputs, digest = _solve_request(manifest_sha256="sha256:" + "a" * 64)
    waiting, _ = store.accept_operation("waiting", SOLVE, digest, target, inputs)
    generation = store.claim("waiting", int(waiting["attempt_generation"]))
    assert generation is not None
    store.record_outcome("waiting", generation, "needs_user_input", reason="setup_required")

    other_target, other_inputs, other_digest = _solve_request(
        manifest_sha256="sha256:" + "b" * 64
    )
    different, _ = store.accept_operation(
        "different", SOLVE, other_digest, other_target, other_inputs
    )
    different_generation = store.claim("different", int(different["attempt_generation"]))
    assert different_generation is not None
    store.record_outcome(
        "different", different_generation, "needs_user_input", reason="setup_required"
    )

    created, outcome, superseded = store.accept_solve_operation(
        "new", digest, target, inputs
    )

    assert outcome == "created" and created["state"] == "received"
    assert [row["operation_id"] for row in superseded] == ["waiting"]
    assert (store.get_operation("waiting")["state"], store.get_operation("waiting")["reason"]) == (
        "cancelled", "superseded"
    )
    assert store.get_operation("different")["state"] == "needs_user_input"


@pytest.mark.parametrize("state", ["received", "processing", "accepted", "rejected", "cancelled"])
def test_a_new_solve_never_supersedes_work_that_is_not_waiting_for_user(
    store: CadLinkStore, state: str
) -> None:
    target, inputs, digest = _solve_request()
    prior, _ = store.accept_operation("prior", SOLVE, digest, target, inputs)
    if state != "received":
        generation = store.claim("prior", int(prior["attempt_generation"]))
        assert generation is not None
        if state != "processing":
            reason = "setup_required" if state == "needs_user_input" else None
            store.record_outcome("prior", generation, state, reason=reason)

    _created, outcome, superseded = store.accept_solve_operation(
        "new", digest, target, inputs
    )

    assert outcome == "created"
    assert superseded == []
    assert store.get_operation("prior")["state"] == state


def test_a_waiting_solve_that_holds_its_own_settings_is_never_superseded(
    store: CadLinkStore,
) -> None:
    # Different operation ids are separate requests. Only a request that never
    # had settings (setup_required) is replaced; one waiting at a later gate
    # carries its own solve intent and stays.
    target, inputs, digest = _solve_request()
    prior, _ = store.accept_operation("prior", SOLVE, digest, target, inputs)
    generation = store.claim("prior", int(prior["attempt_generation"]))
    assert generation is not None
    store.record_outcome(
        "prior", generation, "needs_user_input", reason="frame_confirmation_required"
    )

    _created, outcome, superseded = store.accept_solve_operation("new", digest, target, inputs)

    assert outcome == "created"
    assert superseded == []
    assert store.get_operation("prior")["reason"] == "frame_confirmation_required"


def test_a_superseded_solve_names_the_request_that_replaced_it(store: CadLinkStore) -> None:
    target, inputs, digest = _solve_request()
    prior, _ = store.accept_operation("prior", SOLVE, digest, target, inputs)
    generation = store.claim("prior", int(prior["attempt_generation"]))
    assert generation is not None
    store.record_outcome("prior", generation, "needs_user_input", reason="setup_required")

    store.accept_solve_operation("new", digest, target, inputs)

    outcome = json.loads(store.get_operation("prior")["outcome_json"])
    assert outcome["message"] == "Replaced by solve request new."


def test_recovering_the_same_solve_does_not_supersede_another_operation(
    store: CadLinkStore,
) -> None:
    target, inputs, digest = _solve_request()
    first, _, _ = store.accept_solve_operation("first", digest, target, inputs)
    second, _ = store.accept_operation("second", SOLVE, digest, target, inputs)
    generation = store.claim("second", int(second["attempt_generation"]))
    assert generation is not None
    store.record_outcome("second", generation, "needs_user_input", reason="setup_required")

    recovered, outcome, superseded = store.accept_solve_operation(
        "first", digest, target, inputs
    )

    assert outcome == "recovered" and recovered["operation_id"] == "first"
    assert superseded == []
    assert store.get_operation("second")["state"] == "needs_user_input"
