...
@pytest.mark.parametrize("diagonal_angle", [45.0, 35.0])
def test_metal_auto_uses_full_3d_even_when_axisymmetric_is_available(monkeypatch, diagonal_angle) -> None:
    ...
    async def scenario() -> None:
        outcome = await MetalEngine().run(
            _metal_request(diagonal_angle=diagonal_angle),
        ...
        assert metadata["solve_path_reason"] == "solver_mode='auto' selected native full 3D"
