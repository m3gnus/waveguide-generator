...
def _validated_bundle_asset(value: Any) -> dict[str, Any]:
    ...
    if (
        ...
        or not trusted_asset_url(url)
        or not isinstance(checksum_url, str)
        or not trusted_asset_url(checksum_url)
...
def _validated_cached_release(value: Any) -> dict[str, Any]:
    ...
    if "runtimeId" in value:
        ...
        normalized["bundleAssets"] = [
            _validated_bundle_asset(asset) for asset in bundle_assets
        ]
    return normalized
...
def fetch_recent_releases() -> list[dict[str, Any]]:
    ...
    try:
        with urllib.request.urlopen(  # noqa: S310 - fixed GitHub HTTPS URL
...
def fetch_small_release_asset(url: str, max_bytes: int) -> bytes:
    ...
    if not trusted_asset_url(url):
        raise RuntimeError("GitHub release asset URLs must use HTTPS")
    request = urllib.request.Request(
        url,
        headers={
    ...
    try:
        with urllib.request.urlopen(  # noqa: S310 - URL came from GitHub's release API
            request, timeout=REQUEST_TIMEOUT_SECONDS
        ) as response:
...
def _installed_bundle_identity(app_layer: Path) -> tuple[str, str]:
    try:
        app_manifest = json.loads(
            (app_layer / "APP-MANIFEST.json").read_text(encoding="utf-8")
        )
        runtime_manifest = json.loads(
            (app_layer.parent / "runtime" / "RUNTIME-MANIFEST.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    *,
    ...
    if checkout is not None and checkout.get("kind") == "bundle":
        ...
        for asset in release["bundleAssets"]:
            ...
            if layer == "app" or (
                layer == "runtime" and required_runtime != installed_runtime
            ):
                assets.append(
                    {
                        "name": asset["name"],
...
class UpdateService:
    ...
    @staticmethod
    def _paired_asset(
        uploaded: dict[str, dict[str, Any]], name: str, layer: str
    ) -> dict[str, Any] | None:
    ...
    def _manifest_runtime_id(
        ...
        manifest_bytes = self.asset_fetcher(
            str(manifest_asset["url"]), MAX_MANIFEST_BYTES
        )
    ...
    def _earlier_runtime_asset(self, runtime_id: str) -> dict[str, Any] | None:
        ...
        name = (
            f"waveguide-generator-runtime-{self._bundle_platform()}-{runtime_id}.zip"
        )
        found: dict[str, Any] | None = None
        for release in self._recent_releases:
            found = self._paired_asset(self._uploaded_assets(release), name, "runtime")
    ...
    def _parse_release(
        ...
        *,
        ...
        if checkout is not None and checkout.get("kind") == "bundle":
            ...
            app_asset = self._paired_asset(uploaded, app_name, "app")
            manifest_asset = self._paired_asset(uploaded, manifest_name, "manifest")
            base_release = {
                "version": version,
                "tag": tag,
            ...
            runtime_asset = self._paired_asset(uploaded, runtime_name, "runtime")
            installed_runtime = checkout.get("runtimeId")
            if runtime_asset is None and installed_runtime != runtime_id:
                runtime_asset = self._earlier_runtime_asset(runtime_id)

            dmg_name = f"Waveguide.Generator-{version}-{self._bundle_platform()}.dmg"
            dmg_asset = self._paired_asset(uploaded, dmg_name, "installer")
            bundle_assets = [
                asset
                for asset in (app_asset, manifest_asset, runtime_asset, dmg_asset)
    ...
    def _availability(
        self, cache: dict[str, Any], *, installed_version: str | None = None
    ) -> str:
    ...
    def _refresh(
        self, cache: dict[str, Any], now: float, checkout: dict[str, Any]
    ) -> None:
        ...
        try:
            ...
            if response.not_modified:
                ...
                release, _assets_ready = self._parse_release(
                    response.payload, checkout=checkout
                )
    ...
    def get_status(self, *, force: bool = False) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            cache["availability"] = self._availability(
                cache, installed_version=installed_version
            )
            ...
            availability = (
                str(cache.get("availability") or "unknown")
                if mode_matches
                else "unknown"
            )
            ...
            install_status = (
                ...
                and self.bundle_installer is not None
                else {
                    "installState": "idle",
                    "downloadedBytes": 0,
                    "totalBytes": 0,
    ...
    def request_install(self) -> dict[str, object]:
        ...
        if action.get("kind") == "bundle_download":
            ...
            try:
                progress = installer.start(version, assets)
