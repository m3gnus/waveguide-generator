...
def _extract_mesher_metadata(mesh_result: dict[str, Any]) -> dict[str, Any] | None:
    metadata = mesh_result.get("metadata")
    if isinstance(metadata, dict):
        return metadata

    stats = mesh_result.get("stats")
    if isinstance(stats, dict) and isinstance(stats.get("metadata"), dict):
    ...
    if isinstance(canonical, dict):
        ...
        if isinstance(canonical_metadata, dict):
            mesher_metadata = canonical_metadata.get("mesherMetadata")
            if isinstance(mesher_metadata, dict):
                return mesher_metadata
