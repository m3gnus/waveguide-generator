...
must_haves:
  truths:
    ...
    - "Settings > System section has a Reset All Settings action row (button + help text) that restores all viewer fields to recommended values"
...
resetAllBtn.addEventListener('click', () => {
  resetAllViewerSettings();
  // No live UI update needed here — user would re-open Viewer section to see badge changes
});
```
...
<success_criteria>
Settings > Viewer section shows three sub-sections with all controls. Per-section Reset buttons restore recommended values and update badges. System section has Reset All action. All changes auto-save and live-apply to OrbitControls. `npm test` passes.
</success_criteria>
